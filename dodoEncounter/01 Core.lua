-- ==============================
-- 설정 및 테이블
-- ==============================
---@diagnostic disable: lowercase-global, param-type-mismatch, redundant-parameter, undefined-field, undefined-global
local dodo = _G.dodo
dodoDB = dodoDB or {}

dodo.Encounter = {}

-- NeverSecret → 비교 허용
dodo.Encounter.ENCOUNTER_SOURCE = (Enum and Enum.EncounterTimelineEventSource and Enum.EncounterTimelineEventSource.Encounter) or 0
dodo.Encounter.STATE_FINISHED   = (Enum and Enum.EncounterTimelineEventState  and Enum.EncounterTimelineEventState.Finished)   or 2
dodo.Encounter.STATE_CANCELED   = (Enum and Enum.EncounterTimelineEventState  and Enum.EncounterTimelineEventState.Canceled)   or 3

-- ==============================
-- 캐싱
-- ==============================
local C_CVar            = C_CVar
local C_EncounterEvents = C_EncounterEvents
local CreateFrame       = CreateFrame
local GetCVar           = GetCVar
local SetCVar           = SetCVar
local ipairs            = ipairs

-- ==============================
-- 기능: CVar 보장 & 활성화 확인
-- ==============================
local function ensure_encounter_warnings()
    local cvar = C_CVar and C_CVar.GetCVar and C_CVar.GetCVar("encounterWarningsEnabled")
                 or (GetCVar and GetCVar("encounterWarningsEnabled"))
    if cvar == "0" then
        if C_CVar and C_CVar.SetCVar then C_CVar.SetCVar("encounterWarningsEnabled", "1")
        elseif SetCVar then SetCVar("encounterWarningsEnabled", "1") end
    end
end
dodo.Encounter.EnsureWarnings = ensure_encounter_warnings

function dodo.Encounter.IsEnabled()
    return dodoDB.enableEncounter ~= false
end

-- ==============================
-- SpellMap 공유 캐시 (Sound + TimelineColor 양쪽 공유, PLAYER_ENTERING_WORLD마다 리셋)
-- ==============================
local _spell_map = nil

function dodo.Encounter.GetSpellMap()
    if _spell_map then return _spell_map end
    _spell_map = {}
    if C_EncounterEvents and C_EncounterEvents.GetEventList and C_EncounterEvents.GetEventInfo then
        for _, id in ipairs(C_EncounterEvents.GetEventList()) do
            local info = C_EncounterEvents.GetEventInfo(id)
            if info and info.spellID and info.spellID ~= 0 then
                local t = _spell_map[info.spellID]
                if not t then t = {}; _spell_map[info.spellID] = t end
                t[#t + 1] = id
            end
        end
    end
    return _spell_map
end

function dodo.Encounter.ResetSpellMap()
    _spell_map = nil
end

-- ==============================
-- 이벤트 핸들러
-- ==============================
local initFrame = CreateFrame("Frame")

local function on_event(self, event, arg1)
    if event == "PLAYER_LOGIN" then
        if dodoDB.enableEncounter == nil then dodoDB.enableEncounter = true end
        if dodo.Encounter.IsEnabled() then ensure_encounter_warnings() end
        self:UnregisterEvent("PLAYER_LOGIN")
    elseif event == "PLAYER_ENTERING_WORLD" then
        dodo.Encounter.ResetSpellMap()
    end
end

initFrame:RegisterEvent("PLAYER_LOGIN")
initFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
initFrame:SetScript("OnEvent", on_event)

