...
local CustomCDMConfigs = { -- 물약 지속시간
    ...
    [1236994] = { matchIDs = { 241288, 241289 }, duration = 30, type = 3 },
}
...
dodo.ActionbarInvalidateCDMCache = function() end -- 99 Options.lua 호환용 스텁 (캐시 없음)
...
local function update_cdm_from_item(item)
    ...
    local targetSpellID = (cdmMap and cdmMap[baseSpellID])
    -- CDM spellID와 실제 버프 ID가 다른 경우 (예: 집행자의 정밀함 CDM=386634, 버프=386633)
    -- linkedSpellIDs에서 매핑 탐색
    if not targetSpellID and cdInfo.linkedSpellIDs then
        for _, linkedID in ipairs(cdInfo.linkedSpellIDs) do
            local linkedBase = C_Spell.GetBaseSpell(linkedID)
            targetSpellID = cdmMap and (cdmMap[linkedBase] or cdmMap[linkedID])
            if targetSpellID then break end
        end
    end
    targetSpellID = targetSpellID or baseSpellID
...
-- ==============================
-- 특성 변경 시 새 스펙 기본값 lazy-init + 버튼 캐시 갱신
-- ==============================
local spec_change_frame = CreateFrame("Frame")
spec_change_frame:RegisterEvent("ACTIVE_PLAYER_SPECIALIZATION_CHANGED")
spec_change_frame:SetScript("OnEvent", function()
    if InCombatLockdown() then return end
    get_cdm_map()
    build_special_button_cache()
end)
