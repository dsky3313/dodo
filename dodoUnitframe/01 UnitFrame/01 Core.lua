...
local _scale_to_100 = false
oUF.Tags.Methods['perhp:decimal'] = function(unit)
	local pct = UnitHealthPercent(unit, false, _scale_to_100 or nil)
...
initFrame:SetScript("OnEvent", function(self, event, arg1)
	if event == "ADDON_LOADED" and arg1 == "dodoUnitframe" then
		dodoDB = dodoDB or {}
	elseif event == "PLAYER_LOGIN" then
		if isInitialized then return end
		isInitialized = true

		_scale_to_100 = (CurveConstants and CurveConstants.ScaleTo100) or false
		create_ui()
		update_module_state()
	end
end)
