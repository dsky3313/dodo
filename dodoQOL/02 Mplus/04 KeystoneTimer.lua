...
--
-- SetValue는 킬마다 발생 → 펜딩 큐 배치 처리로 클로저 GC 제거
local pending_pct_bars = {}
local is_pct_scheduled = false

local function process_pct_bars()
    ...
    is_pct_scheduled = false
    if not (dodoDB and dodoDB.enableKeystoneTimer ~= false and dodoDB.useKeystoneTimerPercent ~= false) then
        wipe(pending_pct_bars); return
    end
    ...
    for criteriaIndex = 1, numCriteria do
        ...
        if criteriaInfo and criteriaInfo.isWeightedProgress then
            ...
            if currentQuantity and totalQuantity and totalQuantity > 0 then
                pct_text = string_format("%.2f%%", currentQuantity / totalQuantity * 100)
            end
            break
        end
    end
    ...
    if pct_text then
        for bar in pairs(pending_pct_bars) do
            if bar.Bar and bar.Bar.Label then
                bar.Bar.Label:SetText(pct_text)
            end
        end
    end
    wipe(pending_pct_bars)
end

local function on_progress_bar_set_value(self)
    pending_pct_bars[self] = true
    if not is_pct_scheduled then
        is_pct_scheduled = true
        C_Timer.After(0, process_pct_bars)
    end
end
...
-- 공개 API
-- ==============================
dodo.KeystoneTimerUpdateVisual = update_visual
