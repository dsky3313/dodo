...
local function set_color_from_table(obj, r, g, b, a, isVertex)
    if obj then
        if isVertex then
...
local function party_class_visibility()
    ...
    if isEnabled and pveShown then
        create_icons_if_needed() -- 필요할 때만 아이콘 생성
...
local function update_event_registration()
    ...
    party_class_visibility()
end

-- 파티원 변경 시 UI 업데이트
local function on_party_roster_event(self, event)
    if event == "GROUP_ROSTER_UPDATE" then
        update_ui()
    end
end

partyClassFrame:SetScript("OnEvent", on_party_roster_event)
...
local function on_init_event(self, event, arg1)
    ...
    elseif event == "ADDON_LOADED" and arg1 == "dodoQOL" then
        if PVEFrame then
            PVEFrame:HookScript("OnShow", party_class_visibility)
            PVEFrame:HookScript("OnHide", party_class_visibility)
        end
        self:UnregisterEvent("ADDON_LOADED")
    end
end

initPartyClass:SetScript("OnEvent", on_init_event)

dodo.PartyClass = update_event_registration