...
local _sort_names = {}
local function sort_by_cached_name(a, b) return _sort_names[a] < _sort_names[b] end
...
local function show_tooltip()
    ...
    for i = 1, total do
        ...
        if member_name and is_connected then
            ...
            _sort_names[i] = member_name
        end
    end

    table_sort(temp_members, sort_by_cached_name)
...
local function update_state()
    if dodoDB.useGuildButton == nil then dodoDB.useGuildButton = true end
    
    local is_enabled = (dodoDB and dodoDB.enableChatModule ~= false and dodoDB.useGuildButton ~= false)
    if is_enabled then
