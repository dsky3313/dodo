...
local function on_hyperlink_click(self, link, text, button)
    if dodoDB.enableChatModule and type(link) == "string" and link:sub(1, 4) == "url:" then
...
local function style_chat_frames()
    for i = 1, NUM_CHAT_WINDOWS do
        ...
        if frame then
            if not dodoDB.enableChatModule then
                -- 모듈 비활성화 시 기본값 복구
                ...
                local size = 13
                if dodoDB.useFontSize ~= false then
                    size = dodoDB.fontSize or 13
                end
                local flags = dodoDB.useFontOutline and "OUTLINE" or ""
...
function dodo.UpdateChatFontState()
    local is_enabled = (dodoDB and dodoDB.enableChatModule ~= false)
