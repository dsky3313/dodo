...
local canClearBlizzardSelection = true
local function clearBlizzardSelection()
	if not (EditModeManagerFrame and EditModeManagerFrame.ClearSelectedSystem) then
		return
	end

	if canClearBlizzardSelection then
		local ok = pcall(EditModeManagerFrame.ClearSelectedSystem, EditModeManagerFrame)
		if ok then
			return
		end

		-- Dragonflight/War Within can surface secret-boolean checks in this path.
		canClearBlizzardSelection = false
	end
