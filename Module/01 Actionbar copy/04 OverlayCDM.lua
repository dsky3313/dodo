...
local CDMMapping_defaults = { -- CDM SpellID - Actionbar SpellID (기본값, dodoDB.cdmMapping 없을 때 사용)
...
local function update_cdm_from_item(item)
    ...
    local cdmMap = dodoDB and dodoDB.cdmMapping
    local targetSpellID = (cdmMap and cdmMap[baseSpellID]) or baseSpellID
...
dodo.ActionbarInitCDM = function()
    -- dodoDB.cdmMapping 없으면 기본값으로 초기화
    if dodoDB and not dodoDB.cdmMapping then
        dodoDB.cdmMapping = {}
        for k, v in pairs(CDMMapping_defaults) do
            dodoDB.cdmMapping[k] = v
        end
    end
...
dodo.setCDMMapping = function(buffSpellID, actionBarSpellID)
    ...
    dodoDB.cdmMapping[buffSpellID] = actionBarSpellID
end

dodo.removeCDMMapping = function(buffSpellID)
    if dodoDB and dodoDB.cdmMapping then
        dodoDB.cdmMapping[buffSpellID] = nil
    end
end

-- actionBarSpellID에 연결된 버프 ID 목록 반환 (없으면 빈 테이블)
