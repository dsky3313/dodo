...
-- ==============================
-- ScenarioProgressBarTemplate 기본 장식 (금색 프레임/광채), 리스킨 시 숨김
...
local function create_tick_marks(statusBar)
    ...
    tick2:SetAtlas("honorsystem-bar-frame-exhaustiontick", true)
    tick2:Hide()

    tick3 = statusBar:CreateTexture(nil, "OVERLAY")
    tick3:SetAtlas("honorsystem-bar-frame-exhaustiontick", true)
    tick3:Hide()
end
...
local function on_challenge_mode_activate(self, timerID, elapsedTime, timeLimit)
    ...
    tick2:SetVertexColor(0.3, 0.8, 1)
    tick2:ClearAllPoints()
    tick2:SetPoint("CENTER", statusBar, "LEFT", barW * r2, 0)
    tick2:Show()

    tick3:SetVertexColor(0.4, 1, 0.4)
    tick3:ClearAllPoints()
    tick3:SetPoint("CENTER", statusBar, "LEFT", barW * r3, 0)
    tick3:Show()
end
