...
dodo.RegisterOption("인터페이스", function(category)
    ...
    local _, master_setting = dodo.UI:SettingsCheckbox(category, "enableInterface", "인터페이스 모듈 활성화",
    ...
    -- 피해량 전투기
    T(dodo.UI:SettingsSectionHeader(category, "피해량 전투기"))
    ...
    -- 미니맵
    T(dodo.UI:SettingsSectionHeader(category, "미니맵"))
    ...
    -- 툴팁
    T(dodo.UI:SettingsSectionHeader(category, "툴팁"))
    T(dodo.UI:SettingsCheckbox(category, "useTooltipColor",      "테두리 색상",    "툴팁 테두리 색상을 변경합니다.",                  D.useTooltipColor,      nil))
