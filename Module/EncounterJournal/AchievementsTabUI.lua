...
local function init_row(button, element_data)
    ...
    if not button.InfoIcon then
        ...
        local slot = button:CreateFontString(nil, "OVERLAY", "GameFontBlack")
        slot:SetJustifyH("LEFT")
        slot:SetPoint("BOTTOMLEFT", icon, "BOTTOMRIGHT", 7, 3)
        slot:SetHeight(12)
        button.InfoSlot = slot

        local armorClass = button:CreateFontString(nil, "OVERLAY", "GameFontBlack")
        ...
        local boss = button:CreateFontString(nil, "OVERLAY", "GameFontBlack")
