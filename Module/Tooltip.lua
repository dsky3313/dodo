...
local foundMountName, foundMountIcon
    ...
    if spellID and not isSecret(spellID) then
        ...
        if mountID then
            ...
            if name and name ~= "" then
                foundMountName, foundMountIcon = name, icon
                return true -- 순회 중단
            end
        end
    end
...
local function GetMountInfo(unit)
    ...
    foundMountName, foundMountIcon = nil, nil
    AuraUtil.ForEachAura(unit, "HELPFUL", nil, on_mount_aura_check, true)
    return foundMountName, foundMountIcon
end
...
group:SetScript("OnEvent", on_login_event)
...
table.insert(dodo.OptionRegistrations["interface"], function(category)
    ...
    layout:AddInitializer(CreateSettingsListSectionHeaderInitializer("툴팁"))
    Checkbox(category, "useTooltipHealthHide", "체력바 숨기기", "툴팁 하단의 체력바를 숨깁니다.", true, dodo.UpdateTooltip)
    Checkbox(category, "useTooltipID", "ID 표시", "주문, 아이템 등의 고유 ID를 표시합니다.", true, dodo.UpdateTooltip)
    Checkbox(category, "useTooltipColor", "색상 변경", "직업 및 아이템 등급에 따라 색상을 적용합니다.", true, dodo.UpdateTooltip)
    Checkbox(category, "useTooltipMount", "탈것 정보 표시", "상대방이 타고 있는 탈것 이름을 표시합니다.", true, dodo.UpdateTooltip)
end)